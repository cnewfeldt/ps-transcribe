...
must_haves:
  truths:
    ...
    - "[D-01] ADR documents the chosen library (pointfreeco/swift-snapshot-testing 1.19.2) and three rejected alternatives with rationale, including the Swift Testing trait integration and SwiftPM test-target-only scope"
    - "[D-02] ADR captures the strict precision rationale (precision: 1.0, perceptualPrecision: 0.99, never loosen globally)"
