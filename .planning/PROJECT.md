...
- [x] `QA-FUT-01` — Phase 19 "Looks Done But Isn't" QA checklist sweep + Phase 21 titlebar visual UAT — **Validated in Phase 26: QA Sweep + Visual UAT (2026-05-12)**. `26-UAT.md` documents 10 row dispositions across 9 QA scenarios (QA-03 split into 03a/03b per Phase 24/25 PARTIAL convention). Terminal status `approved-with-debt`. Outcomes: QA-03 PASS (full multi-monitor matrix attested); QA-06..09 pass (citation from `21-HUMAN-UAT.md` Tests 1-4, 2026-05-01 user attestation); QA-01/QA-02/QA-04 WITHDRAWN (QA-01/02: PS Transcribe no longer positions itself as Maccy/Alfred-friendly — privacy markers DICT-09 retained as defensive nspasteboard.org spec compliance only; QA-04: codebase-verified non-applicability of security-scoped bookmarks in non-sandboxed app). QA-05 UNTESTABLE-this-cycle — disk-space preflight unreachable because model manifest URL returns HTTP 404 in production; tracked as `.planning/todos/pending/model-manifest-url-404.md`. Side-effect: Phase 26-02 retired DICT-06 (post-dictation clipboard restore) after the 3s restore was found to break the dictation→paste UX during smoke-test (Task 2). 268/268 tests pass after retirement.
- [ ] `NYQUIST-FUT-01` — Full Nyquist validation sweep across v1.0 phases 1/2/3/8/10 + v1.2 phases 20/21
- [x] `PROCESS-FUT-01` — `requirements_completed` frontmatter on SUMMARY.md template + tooling — **Implemented in Phase 22 (PROCESS-01..03, 2026-05-02); regression closed in Phase 26.1 (2026-05-12)**. Phase 22 introduced the `requirements-completed:` (hyphen, D-01) frontmatter key on SUMMARY.md, the `scripts/lint-summaries.sh` gate, and the `.github/workflows/lint-summaries.yml` CI job. Phase 24's restore of the v1.0-phases archive regressed the gate (PROCESS-03 `passed-then-regressed` — 8 archive SUMMARYs missing the key). Phase 26.1 inserted `requirements-completed: []` (empty list, valid per D-04) into the 8 v1.0-archive SUMMARYs, returning `bash scripts/lint-summaries.sh` to exit 0 (55 files, 0 failures, 0 warnings). `Lint Summaries` CI gate unblocked on `main` for all subsequent PRs.
...
---
*Last updated: 2026-05-12 -- Phase 26.1 (PROCESS-03 lint closure) complete; `Lint Summaries` CI gate green on `main`. `bash scripts/lint-summaries.sh` exits 0 (55 SUMMARY files checked, 0 failures, 0 warnings). 8 v1.0-archive SUMMARYs received `requirements-completed: []` (audit-prescribed closure form; D-04 empty-list valid). Single atomic commit `c6bb92e` + plan SUMMARY commit `851d434`, body bytes untouched (G3 invariant verified). `v1.3-MILESTONE-AUDIT.md` PROCESS-03 row flips `passed-then-regressed → satisfied`; no remaining blockers in the audit's "Recommended Closure Path". Prior context: Phase 26 (QA Sweep + Visual UAT) complete; `QA-FUT-01` validated. `26-UAT.md` ships at `approved-with-debt`: QA-03 PASS (full multi-monitor matrix), QA-06..09 pass-citation from `21-HUMAN-UAT.md`, QA-01/02/04 WITHDRAWN, QA-05 UNTESTABLE-this-cycle blocked by `model-manifest-url-404.md` deferred todo (manifest URL returns HTTP 404 in production — no model updates can ship until release artifacts are republished or the URL is updated). Side-effect: Phase 26-02 retired DICT-06 (post-dictation clipboard restore) after the 3s restore was found to break the dictation→paste UX during smoke-test; 268/268 tests pass after retirement. v1.3 backlog item from code review: stale `clipboardRestoreDelay` UserDefaults key cleanup (one-line edit, not blocking). All four v1.3 milestone pillars now closed: `QA-FUT-01` (✓ Phase 26), `NYQUIST-FUT-01` (✓ v1.0 Phase 24, ✓ v1.2 Phase 25), `PROCESS-FUT-01` (✓ Phase 22 + regression closed in Phase 26.1), `VISREG-01` (✓ Phase 23). Pre-existing CI debt still tracked: Swift 6 strict-concurrency errors in `TranscriptionEngine.swift` and missing Xcode 26 on macos-26 runner — see `.planning/todos/pending/ci-build-check-failing-on-main.md`. **Phase 27 (`gogglebox.com` custom domain) and `DOMAIN-FUT-01` removed from scope 2026-05-05** — site stays on `ps-transcribe-web.vercel.app`. v1.3 Polish & Validation milestone substantively complete; ready for milestone audit + close. Future feature seeds (dictation extensions, library upgrade, transcript editing, format reach) captured in `.planning/seeds/`.*
